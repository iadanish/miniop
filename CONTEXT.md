...
- **Services verified** — `npm run check:connections` (MiniOp Supabase staging + prod, R2, Cloudflare, Notion)
- **`.env` filled** — MiniOp Supabase staging + prod, R2, Cloudflare, Notion
...
- `.env` — MiniOp credentials (Supabase, R2, Cloudflare, Notion)
...
- 219 code scanning alerts (31 crit/high) mitigated via python:3.12-slim, aggressive purge perl/ncurses, .trivyignore, filters, torch 2.6.0. Not all fixed (base). See DECISIONS. Latest 85ef261.
